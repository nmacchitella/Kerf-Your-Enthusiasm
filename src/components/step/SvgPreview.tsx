...
function layerStrokeWidth(layer?: string): string {
...
export function SvgPreview({ edgeData }: { edgeData: EdgeData }) {
  const { viewBox, paths } = useMemo(() => {
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      const strokeWidth = layerStrokeWidth(e.layer);
      const priority = layerDrawPriority(e.layer);
      if (e.type === 'line') {
        result.push({ d: `M ${e.start[0]} ${flip(e.start[1])} L ${e.end[0]} ${flip(e.end[1])}`, color, strokeWidth, key: i, priority });
      } else if (e.type === 'arc') {
        if (e.is_full_circle) {
          result.push({ d: `M ${e.center[0] - e.radius} ${flip(e.center[1])} A ${e.radius} ${e.radius} 0 1 0 ${e.center[0] + e.radius} ${flip(e.center[1])} A ${e.radius} ${e.radius} 0 1 0 ${e.center[0] - e.radius} ${flip(e.center[1])}`, color, strokeWidth, key: i, priority });
        } else {
          ...
          result.push({ d: `M ${sx} ${sy} A ${r} ${r} 0 ${sweep > 180 ? 1 : 0} 0 ${ex} ${ey}`, color, strokeWidth, key: i, priority });
        }
      } else if (e.type === 'polyline') {
        const pts = e.points; if (pts.length < 2) continue;
        result.push({ d: pts.map(([x, y], idx) => `${idx === 0 ? 'M' : 'L'} ${x} ${flip(y)}`).join(' '), color, strokeWidth, key: i, priority });
      }
    }
